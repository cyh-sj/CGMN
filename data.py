...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt):
        ...
        self.sizes = np.load(loc + '%s_ims_size.npy' %
                             data_split, allow_pickle=True)
        self.IOU = np.load(loc + '%s_IOU_half.npy' %
                             data_split, allow_pickle=True)
    ...
    def __getitem__(self, index):
        ...
        for i in range(k):
            for j in range(k):
                Iou[i,j] = self.getIou(bboxes[i], bboxes[j])'''
        ...
        #return image, target, index, img_id, caption_label, caption_mask, depend, bboxes
        return image, target, index, img_id, depend, bboxes, IOU, caption_label, caption_mask
    '''
...
def collate_fn(data):
    ...
    #images, captions, ids, img_ids, depend = zip(*data)
